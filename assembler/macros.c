...
int parse_data (FILE *input_fp, int code_pc) {
                ...
                if (line_words[0][0]=='#') {
                        ...
                        if(strlen(line_words[1])==0) {
                                ...
                                if(strlen(remain_str)>=STR_LEN) {
                                        ...
                                        data = malloc(strlen(remain_str)*8+1);
                                        data[0]=0;
                                        for (int i=0; i<strlen(remain_str); i++) {
                                                sprintf(temp_string,"000000%02X",remain_str[i]);
                                                strcat(data,temp_string);
                                        }
                                        data[strlen(remain_str)*8]=0;
        ...
        data = malloc(5);
        data[0]=0;
        sprintf(data,"%04X",stack_size);
        add_data_element(".Stack","Stack",4,data,code_pc);
        return(0);
}
